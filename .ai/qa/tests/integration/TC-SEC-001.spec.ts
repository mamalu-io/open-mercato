...
/**
 * TC-SEC-001: Admin password reset revokes all active customer sessions
 *
 * Regression test for: admin-initiated password reset did not invalidate
 * active portal sessions, allowing the session token to be used to refresh
 * the JWT and maintain access after a forced password reset.
 *
 * Architecture: customer_auth_token is a short-lived JWT (15 min).
 * customer_session_token is stored in DB and validated on refresh.
 * After revokeAllUserSessions, sessions-refresh must return 401.
 */
test.describe('TC-SEC-001: Admin password reset revokes active customer sessions', () => {
  test('sessions-refresh should be blocked after admin resets the password', async ({ request }) => {
    const stamp = Date.now()
    const customerEmail = `qa-sec-001-${stamp}@test.local`
    ...
    try {
      ...
      const portalLoginRes = await request.post(`${BASE_URL}/api/customer_accounts/login`, {
      ...
      const refreshBeforeRes = await request.post(`${BASE_URL}/api/customer_accounts/portal/sessions-refresh`, {
      ...
      // The JWT may still be valid (short TTL, up to 15 min), but the session token
      // is revoked in DB so the attacker cannot renew access beyond the current JWT window.
      const refreshAfterRes = await request.post(`${BASE_URL}/api/customer_accounts/portal/sessions-refresh`, {
        headers: { Cookie: authCookie },
      })
      expect(
